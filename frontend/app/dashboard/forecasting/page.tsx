'use client';

import { useForecasting } from '@/hooks/useForecasting';
import { LoadingSpinner } from '@/components/LoadingStates';
import { ErrorAlert } from '@/components/ErrorStates';
import { TrendingUp, BarChart3, Calendar, RefreshCw } from 'lucide-react';

export default function ForecastingPage() {
    const { data: forecasts, loading, error, refetch } = useForecasting();

    const stats = {
        totalForecasts: forecasts?.length || 0,
        accuracy: forecasts?.reduce((sum, f) => sum + (f.accuracy || 0), 0) / (forecasts?.length || 1) || 0,
        nextPeriod: forecasts?.[0]?.period || 'N/A'
    };

    return (
        <>
            {loading && <LoadingSpinner />}
            {error && <ErrorAlert message={error} />}
            {!loading && !error && (
                <div className="p-6 bg-gray-900 min-h-screen text-gray-100">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h1 className="text-3xl font-bold flex items-center gap-2">
                                <TrendingUp className="h-8 w-8 text-purple-400" />
                                Sales Forecasting
                            </h1>
                            <p className="text-gray-400 mt-1">Predictive analytics and trends</p>
                        </div>
                        <button onClick={() => refetch()} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg transition">
                            <RefreshCw className="h-4 w-4" />
                            Refresh
                        </button>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                            <div className="flex items-center justify-between mb-2">
                                <p className="text-sm text-gray-400">Total Forecasts</p>
                                <BarChart3 className="h-5 w-5 text-purple-400" />
                            </div>
                            <p className="text-2xl font-bold">{stats.totalForecasts}</p>
                        </div>
                        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                            <div className="flex items-center justify-between mb-2">
                                <p className="text-sm text-gray-400">Accuracy</p>
                                <TrendingUp className="h-5 w-5 text-green-400" />
                            </div>
                            <p className="text-2xl font-bold">{stats.accuracy.toFixed(1)}%</p>
                        </div>
                        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                            <div className="flex items-center justify-between mb-2">
                                <p className="text-sm text-gray-400">Next Period</p>
                                <Calendar className="h-5 w-5 text-blue-400" />
                            </div>
                            <p className="text-lg font-bold truncate">{stats.nextPeriod}</p>
                        </div>
                    </div>
                </div>
            )}
        </>
    );
}
